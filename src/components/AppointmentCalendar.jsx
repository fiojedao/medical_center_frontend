...
const AppointmentCalendar = ({onData, doctor_id}) => {
  const localizer = momentLocalizer(moment);  
  const [myEvents, setEvents] = useState([]);
  const [id, setID] = useState(null);
  const [start, setStart] = useState(false);
  const { data, error, loaded } = useCallApi({ endpoint: doctor_id ? `appointments/getbydoctor/${doctor_id}`:'appointments' });
  const { responseData, errorData, loadedData } = useSubmitForm({ endpoint: `appointments`, action: "DELETE", formData: {id: Number(id)}, start });
  ...
  const handleSelectEvent = useCallback(
    (event) => {
      ...
      if(confirm){
        ...
        let i = myEvents.findIndex(objeto => objeto.title === event.title && objeto.start === event.start && objeto.end === event.end);
        myEvents.splice(i, 1);
      }
    },
    [myEvents]
